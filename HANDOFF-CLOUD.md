...
- **Native track — STARTED (user said go). Increments, each device-tested:**
  ...
  - **Device-test round 2 (`d980fb4`).** User: native export is "blazing fast" ✅ but (a) captions/text NOT
    baked, (b) native preview player buggy — audio double-plays for the first ~5 s, playhead sticks+vibrates at
    start, can't scrub while playing, seam stutter. Fixes: (a) **captions bake from the LIVE doc** —
    `nativeExportFull` now calls `bakeCaptions(doc, project, W, H)` with `documentToProject(doc,…)` (the
    shared-engine doc used for the segments); it was reading `project.timeline` which is empty in doc mode, so
    zero texts were baked. (b) **native preview player OFF by default** (`useNativePreview.enabled()` now needs
    `localStorage ec.nativePlayer==='1'`) → the proven HTML `<video>` preview runs, killing the double-audio /
    stuck-start / no-scrub regressions. Run `30000502382` v13 GREEN; APK 37.6 MB. **STILL OPEN:** (1) verify
    captions actually render in the exported mp4 on-device (baking is fixed; the Media3 OverlayEffect
    composition-time/position is still unverified — if captions show but mistimed/mispositioned, that's the next
    fix). (2) image/video b-roll overlays not baked. (3) mobile preview seam stutter (single hardware decoder)
    is back to the known HTML baseline — the native player is the real fix but needs proper device iteration
    (start latency, A/V start sync, scrub-during-play, ExoPlayer playlist seam transitions).
