...
const MODEL_WHITELIST = new Set([
  ...
  // DeepSeek first-party (un-prefixed ids)
  'deepseek-v4-flash',
  'deepseek-v4-pro',
  'deepseek-chat',
...
async function callModel(
  ...
  if (provider === 'openrouter') {
    headers['HTTP-Referer'] = 'https://easecutpro.com'
    headers['X-Title'] = 'EaseCutPro Ultracut'
  }
  ...
  try {
    ...
    await logDebug({ model, provider, aborted: true, err: (e as Error).message })
