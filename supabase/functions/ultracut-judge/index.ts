...
//  • TIME BUDGET: the primary is aborted at PRIMARY_BUDGET_MS (< 400s).
//  • SINGLE MODEL: Strictly locked to Claude 3.5 Sonnet via OpenRouter.
...
async function callModel(
  ...
  try {
    ...
    await logDebug({ model, provider: 'openrouter', aborted: true, err: (e as Error).message })
...
async function finalize(
  ...
  try {
    primary = await callModel(apiKey, model, payload, system, ctrl.signal)
  } finally {
    clearTimeout(timer)
  }
  ...
  throw new Error(`model API failed or timed out (HTTP ${primary.status})`)
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { payload, promptVariant } = await req
      .json()
      .catch(() => ({ payload: null, promptVariant: null }))
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        let out: unknown
        try {
          const r = await finalize(apiKey, model, payload, system)
