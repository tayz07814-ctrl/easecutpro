...
/**
 * The FSMN (FunASR) VAD engine — the native ONNX port of the desktop web
 ...
 * ":vadengine"). The decoder and the ONNX runtime are native code; a native fault
 * (SIGSEGV/SIGABRT) in either cannot be caught from Kotlin and previously took the
 * whole app down at "Cleaning silence…". Isolated in a helper process, the same
 * fault now kills only the helper: the app's binder call throws, EcVad reports the
 * stage recorded in [stageFile] and falls back to transcript word-gap silence. The
 * app can no longer crash from this engine, by construction.
 *
 ...
 * and the Flutter engine. So: the decode resamples to 16 kHz inline and keeps 16-bit
 * PCM (never the source-rate float copy), features are computed one CHUNK_FRAMES
 * window at a time straight into the tensor, and the tensors' direct buffers are
 * allocated once and reused across windows. Isolation helps here too — the helper's
 * working set no longer stacks on top of the preview player's.
 *
...
object VadEngine {
    ...
    /**
     * Full pipeline: decode → fbank/LFR/CMVN → FSMN → endpoint machine → Final Boss
     * geometry. Returns silence regions [startS, endS] (seconds) to REMOVE. Throws on
     * any CATCHABLE failure (the caller reports it); a native fault kills this
     * process, and the caller learns the stage from [stageFile].
     */
    fun detect(
        context: Context,
        uri: String,
        ...
        val out = if (tailTrim) trimQuietTails(planned, pcm, count) else planned
        stage(context, "done")
        return out
    }
    ...
    private fun ensureRuntime(context: Context) {
        ...
        synchronized(this) {
            if (session == null) {
                ...
                val ioValid = ins.contains("speech") && outs.contains("logits") &&
                ...
                if (!ioValid) {
                    ...
                    throw IllegalStateException(
                        "model i/o unexpected: in=[${ins.joinToString(",")}] out=[${outs.joinToString(",")}]"
                    )
                }
            }
            if (cmvnMeans == null) {
     ...
     */
    private fun detectFsmnSilences(
        context: Context,
        pcm: ShortArray,
        count: Int,
        durationS: Double
    ): List<DoubleArray> {
        stage(context, "onnx model load")
        ...
        // the Java heap) — exactly the kind of native growth the low-memory killer
        // answers with SIGKILL.
        ...
        val totalWindows = (frames + CHUNK_FRAMES - 1) / CHUNK_FRAMES
        var start = 0
        while (start < frames) {
            val n = min(CHUNK_FRAMES, frames - start)
            stage(context, "features window ${start / CHUNK_FRAMES + 1}/$totalWindows")
            ...
            stage(context, "onnx inference window ${start / CHUNK_FRAMES + 1}/$totalWindows")
            speechBuf.clear()
    ...
    private const val LFR_M = 5
    private const val FEATURE_DIM = MEL_BINS * LFR_M      // 400
    private const val CACHE_LAYERS = 4
    private const val CACHE_PROJ = 128
    private const val CACHE_ORDER = 19
    // 10 s per inference window. The FSMN caches make this exact, so the only
    // thing a bigger window buys is a bigger native arena.
    private const val CHUNK_FRAMES = 1000
    private const val LOGITS_STRIDE = 248                 // speechProb = 1 - logits[i*248]
    ...
    private const val SPEECH_THRESHOLD = 0.8f
    private const val WINDOW_FRAMES = 20
    private const val TRANSITION_FRAMES = 15
    private const val START_LOOKBACK_FRAMES = 20
    private const val END_SILENCE_FRAMES = 65
    private const val END_LOOKAHEAD_FRAMES = 10
    // Retake Final Boss fixed endpoint cushions + tail-trim.
    private const val AFTER_SPEECH_CUSHION_S = 0.16
    private const val BEFORE_SPEECH_CUSHION_S = 0.26
    private const val TAIL_TRIM_DB = -22.0
    private const val TAIL_TRIM_MAX_S = 0.2
    private const val TAIL_TRIM_STEP_S = 0.02
}
