...
/** Deliver a completed browser export to the user.
 *
 * Browsers do not expose the final filesystem path. On browsers that support
 * showSaveFilePicker we try a real Save dialog first; otherwise we fall back to
 * an anchor download. The fallback is intentionally best-effort and works on
 * Safari/iOS where the File System Access API is unavailable.
 */
export async function deliverBrowserExport(blob: Blob, name: string): Promise<string | null> {
  const w = typeof window !== 'undefined' ? window : null

  // Chromium desktop: offer an actual Save As dialog when available. Do not let
  // picker cancellation turn a successful export into a failed export.
  try {
    const picker = (w as Window & { showSaveFilePicker?: (options?: unknown) => Promise<{ createWritable: () => Promise<{ write: (data: Blob) => Promise<void>; close: () => Promise<void> }> }> }).showSaveFilePicker
    if (picker) {
      ...
      try {
        ...
        try { await writable.close() } catch { /* ignore cleanup */ }
        throw e
      }
      return 'saved'
    }
  } catch (e) {
    // Abort/capability errors are normal. Fall through to a normal download.
  }
  ...
  try {
    ...
    // Keep the object URL alive long enough for Safari/iOS to consume the click.
    setTimeout(() => {
...
export async function exportOnDevice(
  ...
  try {
    ...
    await deliverBrowserExport(blob, name)
    return { blob, name }
  } catch (e) {
