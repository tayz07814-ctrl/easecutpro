    ...
    <div onClick={onOpen} style={css('display:flex;gap:12px;align-items:center;padding:10px;background:#1E2026;border:1px solid rgba(255,255,255,.07);border-radius:13px;position:relative;cursor:pointer')}>
      ...
      <div style={css('width:76px;height:50px;flex:none;border-radius:8px;overflow:hidden;background:' + HATCH + ';display:grid;place-items:center')}>
        {card.kind === 'video' && thumb ? (
          ...
          <span style={css('font-size:11px;font-weight:600;color:#9BA0AC')}>{card.percent}%</span>
        ) : card.kind === 'failed' ? (
          <div style={css('width:22px;height:17px;border:1.5px solid #4A4F5B;border-radius:4px;display:grid;place-items:center')}>
            <div style={css('width:0;height:0;border-left:6px solid #4A4F5B;border-top:4px solid transparent;border-bottom:4px solid transparent;margin-left:2px')} />
          </div>
        ) : (
          <span style={css("font-family:'IBM Plex Mono',monospace;font-size:9.5px;color:#686E7B")}>16:9</span>
        )}
      </div>
      ...
      <div style={css('flex:1;min-width:0')}>
        {renaming ? (
          <input
            ...
            style={css('font-size:14px;font-weight:550;width:100%;background:#141519;color:#E9EAEE;border:1px solid rgba(110,106,232,.55);border-radius:6px;padding:3px 7px;font-family:inherit;outline:none')}
          />
        ) : (
          <div style={css('font-size:14px;font-weight:550;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>{title}</div>
        )}
        <div style={css('font-size:12px;color:#9BA0AC;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>{sub}</div>
        {card.kind === 'processing' && (
          <div style={css('width:100%;height:3px;border-radius:2px;background:#2A2D36;overflow:hidden;margin-top:7px')}>
      ...
      {/* Dots menu */}
      <div onClick={onDots} style={css('flex:none;color:#9BA0AC;font-size:18px;line-height:1;padding:6px 8px;border-radius:8px;cursor:pointer', menuOpen && 'background:#262932')}>···</div>
      {menuOpen && (
        <div onClick={(e) => e.stopPropagation()} style={css('position:absolute;right:10px;top:calc(100% - 6px);width:170px;background:#262932;border:1px solid rgba(255,255,255,.1);border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,.55);padding:6px;z-index:8')}>
          ...
          <div onClick={onDelete} style={css('padding:10px;font-size:14px;border-radius:8px;color:#D9686E;cursor:pointer')}>Delete</div>
        </div>
      )}
    </div>
  )
...
export default function MobileDashboard(): JSX.Element {
  ...
  return (
    <div
      ...
      style={css('width:100%;min-height:100dvh;background:#17181C;display:flex;flex-direction:column')}
      onClick={() => { setMenuId(null); setAcct(false) }}
    >
      {/* Top bar (sticky) */}
      <div style={css(`flex:none;display:flex;align-items:center;gap:10px;height:54px;padding:0 16px;border-bottom:1px solid ${HAIR};position:sticky;top:0;background:#17181C;z-index:10`)}>
        ...
        <div style={css('position:relative')} onClick={(e) => { e.stopPropagation(); setAcct((v) => !v) }}>
          ...
          {acct && (
            <div onClick={(e) => e.stopPropagation()} style={css('position:absolute;right:0;top:42px;width:210px;background:#1E2026;border:1px solid rgba(255,255,255,.09);border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,.5);padding:6px;z-index:20')}>
              <div style={css('padding:8px 10px 6px;font-size:12px;color:#9BA0AC;border-bottom:1px solid rgba(255,255,255,.06);margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>{dash.email}</div>
              <div onClick={() => void dash.logout()} style={css('padding:9px 10px;font-size:14px;border-radius:8px;color:#9BA0AC;cursor:pointer')}>Log out</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={css('flex:1;padding:18px 16px 40px')}>
        ...
        <div style={css('display:flex;align-items:center;gap:10px;height:44px;padding:0 13px;background:#1E2026;border:1px solid rgba(255,255,255,.07);border-radius:12px;margin-bottom:18px')}>
          <div style={css('width:13px;height:13px;border:1.5px solid #686E7B;border-radius:50%;position:relative;flex:none')}>
          ...
          <input
            ...
            style={css('font-size:14.5px;color:#E9EAEE;flex:1;background:none;border:none;outline:none;font-family:inherit;min-width:0')}
          />
        </div>
        ...
        <div style={css('display:flex;flex-direction:column;gap:10px')}>
          {dash.metas.length === 0 && (
            <div style={css('color:#686E7B;font-size:14px;text-align:center;padding:44px 0')}>
