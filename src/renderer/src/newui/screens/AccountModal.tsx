  ...
  useEffect(() => {
    ...
    const off = onPaddleEvent(async (name) => {
      if (name === 'checkout.completed') {
        // Webhook writes the sub a beat later — poll until Pro shows, then confirm.
      ...
      if ((name === 'checkout.completed' || name === 'checkout.closed') && active) setBusy(null)
    })
    return () => {
      active = false
      off()
          ...
          {!isPro && (
            <>
              ...
              <div style={{ fontSize: '12px', color: '#6E7280', marginTop: '12px' }}>
                Secure checkout by Paddle · Cancel anytime · 14-day money-back guarantee
              </div>
            </>
          )}
        </div>
