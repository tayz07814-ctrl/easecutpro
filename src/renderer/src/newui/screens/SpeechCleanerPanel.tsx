  ...
  if (r.state === 'idle') {
    return (
      <div style={css('flex:1;min-height:0;overflow-y:auto;padding:14px 14px 18px')}>
        ...
        {/* Silence ONLY — the same FSMN engine and Silence settings Find cuts uses,
            without transcription or the retake judge. Secondary styling so
            "Find cuts" stays the primary action. */}
  ...
  return (
    <div style={css('flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden')}>
      ...
      <div style={css(`flex:none;padding:12px 14px;border-bottom:1px solid ${HAIR}`)}>
        ...
        {/* Silence-only re-scan, reachable alongside Retake results. Re-running
            reflects the latest Silence settings. */}
        {!applied && (
