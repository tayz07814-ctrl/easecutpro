  ...
  return (
    <button
      ...
      style={css(
        ...
        `background:${CARD_BG};`,
        on ? `border:1.5px solid ${ACCENT};box-shadow:0 0 0 3px rgba(110,106,232,.18)` : `border:1.5px solid ${HAIR}`,
    ...
    >
      ...
      <div style={css('flex:1;min-width:0')}>
        <div style={css('font-size:13.5px;font-weight:600;color:#EAECF1;display:flex;align-items:center;gap:7px')}>
          {def.title}
          {def.soon && (
            <span style={css('font-size:9.5px;font-weight:700;letter-spacing:.4px;color:#B9B4FF;background:rgba(110,106,232,.18);border-radius:5px;padding:2px 6px;text-transform:uppercase')}>
              Soon
            </span>
          )}
        </div>
        <div style={css('font-size:11.5px;color:#8B909C;margin-top:2px')}>{def.sub}</div>
      </div>
      {/* check circle */}
      <div
        style={css(
  ...
  return (
    <div
      ...
      style={css(
        ...
        'background:rgba(8,9,12,.66);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)'
      )}
    >
      <div
        ...
        style={css(
          ...
          'background:#131418;border:1px solid rgba(255,255,255,.09);border-radius:18px;',
      ...
      >
        ...
        <div style={css('display:flex;align-items:flex-start;justify-content:space-between;gap:10px')}>
          <div>
            <div style={css('font-size:17px;font-weight:650;color:#F2F3F6')}>New project</div>
            <div style={css('font-size:12px;color:#8B909C;margin-top:3px')}>
              Import your footage and pick what to auto-apply. You can change everything later.
            </div>
          </div>
          {!processing && (
            <div onClick={onClose} style={css('color:#9BA0AC;font-size:20px;line-height:1;cursor:pointer;padding:2px 6px;border-radius:8px')}>
              ×
            </div>
          )}
        </div>
        ...
        {processing ? (
          <div style={css('padding:34px 6px 18px')}>
            <div style={css('font-size:13px;font-weight:600;color:#EAECF1;margin-bottom:14px')}>
              {wizardJob?.label ?? 'Preparing…'}
            </div>
            <div style={css('height:10px;border-radius:6px;background:#23242B;overflow:hidden')}>
              <div
                style={css(
                  `width:${shownPct}%;height:100%;border-radius:6px;transition:width .18s ease-out;`,
                  `background:linear-gradient(90deg,${ACCENT},#8E8BF0)`
                )}
              />
            </div>
            <div style={css('display:flex;justify-content:space-between;margin-top:9px')}>
              <span style={css('font-size:11.5px;color:#8B909C')}>This can take a minute for longer videos…</span>
              <span style={css("font-size:12px;font-weight:600;color:#B9B4FF;font-family:'IBM Plex Mono',monospace")}>{shownPct}%</span>
            </div>
          </div>
        ) : (
          <>
            ...
            <div style={css('margin-top:18px')}>
              <button
                ...
                style={css(
                  'width:100%;font-family:inherit;font-size:13px;font-weight:600;color:#C9CBD3;cursor:pointer;',
                  'background:rgba(110,106,232,.08);border:1.5px dashed rgba(110,106,232,.5);border-radius:12px;padding:16px'
                )}
              >
                ...
                <div style={css('margin-top:10px;display:flex;flex-direction:column;gap:6px')}>
                  {files.map((f, i) => (
                    <div
                      key={f.path}
                      style={css(
                        'display:flex;align-items:center;gap:10px;background:#191A20;border:1px solid rgba(255,255,255,.06);',
                        'border-radius:9px;padding:8px 11px'
                      )}
                    >
                      <span style={css("font-size:10px;color:#6E6AE8;font-weight:700;font-family:'IBM Plex Mono',monospace;width:16px")}>{i + 1}</span>
                      <span style={css('flex:1;min-width:0;font-size:12.5px;color:#DADCE2;overflow:hidden;text-overflow:ellipsis;white-space:nowrap')}>{f.name}</span>
                      <span onClick={() => removeFile(f.path)} style={css('color:#8B909C;font-size:15px;cursor:pointer;line-height:1;padding:0 2px')}>×</span>
                    </div>
                  ))}
                  {files.length > 1 && (
                    <div style={css('font-size:11px;color:#8B909C;margin-top:2px')}>
            ...
            <div style={css('margin-top:20px;display:flex;flex-direction:column;gap:9px')}>
              <div style={css('font-size:11px;font-weight:600;letter-spacing:.5px;text-transform:uppercase;color:#7C818D;margin-bottom:1px')}>
                Enhance on import
              </div>
              {OPTIONS.map((def) => (
            ...
            <div style={css('display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:22px')}>
              <button
                type="button"
                onClick={onClose}
                style={css('font-family:inherit;font-size:13px;font-weight:600;color:#C6C9D2;background:transparent;border:none;cursor:pointer;padding:10px 14px')}
              >
                Cancel
              </button>
              <button
                ...
                style={css(
                  ...
                  files.length
                    ? `background:${ACCENT};cursor:pointer;box-shadow:0 6px 20px rgba(110,106,232,.35)`
                    : 'background:#2A2B33;color:#6B6F79;cursor:not-allowed'
                )}
              >
                {ctaLabel} →
              </button>
